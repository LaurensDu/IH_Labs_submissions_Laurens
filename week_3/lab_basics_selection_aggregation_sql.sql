...
SELECT loan_id, max(payments) FROM bank.loan;
...
SELECT DISTINCT count(card_id), type FROM bank.card
GROUP BY type
ORDER BY type DESC;
...
SELECT account_id, sum(amount) as sum_loan FROM bank.loan
GROUP BY account_id
...
SELECT order_id, count(loan_id)
WHERE order_id < 930907
GROUP BY date DESC;

# Query 17
# In the `loan` table, for each day in December 1997, count the number of loans issued for each unique loan duration, ordered by date and duration, both in ascending order. You can ignore days without any loans in your output.

SELECT distinct(duration), count(distinct(loan_id)), date FROM bank.loan
